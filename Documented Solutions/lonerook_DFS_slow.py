# Title: Lone Rook with DFS (Too Slow)
# Author: Gubanyi
# Date: 10/21/2024
#
# Description: Implements a DFS to solve the Lone Rook problem.
#              Too slow: each time a knight is killed, we restart the search.
#
...
rows = int(inputs[0])
cols = int(inputs[1])
board = [list(input()) for i in range(rows)]
...
def update_attacks(i, j):
    # (i,j) is the location where a knight was killed
    # Update the attacks based on that knight being removed
    if i-1 >= 0 and j-2 >= 0:
        A[i-1][j-2] -= 1
    ...
    while stack:
        ...
        while new_i < rows and not hit_knight:
            ...
            if is_valid(new_i, j):
                if hit_knight:
                    board[new_i][j] = "."
                    update_attacks(new_i,j)
                    # kill knight and restart
                    stack = [start]
                    visited = set()  # to track visited nodes
                    visited.add(start)
                else:
                    stack.append((new_i, j))
                    visited.add((new_i, j))
        ...
        while new_j >= 0 and not hit_knight:
            ...
            if is_valid(i, new_j):
                if hit_knight:
                    board[i][new_j] = "."
                    update_attacks(i,new_j)
                    # kill knight and restart
                    stack = [start]
                    visited = set()  # to track visited nodes
                    visited.add(start)
                else:
                    stack.append((i, new_j))
                    visited.add((i, new_j))
